...
class Geometry {
    ...
    addVertex(...args) {
        for (let index = 0; index < args.length; index += 3) {
            this.vertices.push(new Vertex(args[index], args[index + 1], args[index + 2]))
        }
        return this
    }
    ...
    get normalData() {
        return new Float32Array(this.vertices.flatMap((vertex) => Array.from(vertex.normal)))
    }
...
function makeVertexArray(gl, bufferLocNumElmPairs, indices) {
    const vertexArray = gl.createVertexArray()
    gl.bindVertexArray(vertexArray)

    for (const [buffer, loc, numElem] of bufferLocNumElmPairs) {
        if (loc === -1) continue
            ...
            {activeItem && (
                <>
                    ...
                    <p
                        ...
                        ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
                        ${isMoving
                                ? 'pointer-events-none opacity-0 duration-100 translate-x-[-60%] -translate-y-1/2'
                                : 'pointer-events-auto opacity-100 duration-500 translate-x-[-90%] -translate-y-1/2'
                            }
                    `}
                    >
                    ...
                    <div
                        ...
                        ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
                        ${isMoving
                                ? 'pointer-events-none -bottom-20 opacity-0 duration-100 scale-0 -translate-x-1/2'
                                : 'pointer-events-auto bottom-[3.8em] opacity-100 duration-500 scale-100 -translate-x-1/2'
                            }
                    `}
                    >
                        <p className="relative top-0.5 select-none text-[26px] text-[#060010]">&#x2197;</p>
                    </div>
                </>
            )}
        </div>
